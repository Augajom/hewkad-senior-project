...
const OrderingCard = ({ order, onStatusUpdate }) => {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      const res = await fetch(`${API_URL}/service/orders/${order.id}/upload-proof`, {
        method: "POST",
        body: formData,
      ...
      if (res.ok) {
        ...
        if (onStatusUpdate) onStatusUpdate(order.id, data.status, `${API_URL}${data.proof_url}`);
      } else {
    ...
    <div className="relative card w-full bg-white shadow-lg rounded-xl border border-gray-200 p-4 text-black">
      ...
      <div className="flex justify-between items-start">
        <div className="flex gap-3">
          ...
          />
          <div>
            <div className="font-bold text-base">{order.nickname || order.name || "ไม่ระบุชื่อ"}</div>
            <div className="text-sm text-gray-500">{order.username || "@username"}</div>
          </div>
        </div>
      ...
      {order.proof_url && (
        <div className="mt-2">
          ...
          <img src={order.proof_url} alt="Proof" className="w-full max-h-48 object-contain rounded-lg mt-1 border" />
        </div>
      )}
      ...
      {order.status_name === "Ordering" && (
        <div className="mt-4 flex justify-end">
          <button onClick={handleSendClick} className="btn btn-primary text-white text-sm px-4 py-1">Send</button>
...
const OrderingList = () => {
  const { orders: ordersFromHook, loading, error } = useOrders("Rider Received,Ordering,Order Received,Reported");
