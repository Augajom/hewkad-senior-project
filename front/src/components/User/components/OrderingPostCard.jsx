...
const OrderingPostCard = ({ post }) => {
  ...
  // Upload slip
  const handleFile = (e) => setSlipFile(e.target.files[0]);
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Failed to upload slip: ${res.status}`);
      const data = await res.json();
      setSlipOkData(data);
  ...
  return (
    <div className="card w-full max-w-sm bg-white shadow-lg rounded-xl border border-gray-200 p-6 text-black">
      {/* Header */}
      ...
      <div className="mt-4 text-sm space-y-1">
        ...
        <p>
          <span className="font-semibold">Store Name</span>: {post.store_name || "-"}
        </p>
        <p>
      ...
      {showQRModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg w-80 space-y-4 text-center shadow-xl">
            ...
            {qrCode ? <img src={qrCode} alt="PromptPay QR" className="mx-auto" /> : <p>กำลังโหลด...</p>}
            <div>
              Total Amount: <span className="font-bold text-lg">{total} ฿</span>
            </div>
            <button onClick={handleOpenSlipModal} className="btn btn-success w-full mt-4">
              แนบสลิปการชำระเงิน
            </button>
      ...
      {showSlipModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg w-96 space-y-4 text-center shadow-xl">
            <h2 className="text-xl font-bold text-black">แนบสลิปการชำระเงิน</h2>
            <form onSubmit={handleSubmit}>
              <input type="file" accept="image/*" id="slipUpload" onChange={handleFile} className="hidden" />
              <label htmlFor="slipUpload" className="btn btn-outline btn-primary w-full cursor-pointer">
                เลือกไฟล์
              </label>
              {slipFile && (
                <img src={URL.createObjectURL(slipFile)} alt="Slip Preview" className="mx-auto mt-3 rounded-lg max-h-60 object-contain" />
              )}
              {slipError && <p className="text-red-500 text-sm mt-2">{slipError}</p>}
              <button className="btn btn-success w-full mt-4" type="submit">
                ส่งสลิป
              </button>
              <button onClick={() => setShowSlipModal(false)} className="btn btn-ghost w-full mt-2">
                ยกเลิก
              </button>
            </form>
          </div>
