...
const HistoryPostCard = ({ post, className = "" }) => {
  ...
  useEffect(() => {
    const fetchReasons = async () => {
      try {
        const res = await fetch("https://hewkad.com:2053/customer/report-reasons", {
          credentials: "include",
        });
    ...
    <div className={`card w-full sm:w-[450px] md:w-[400px] lg:w-[450px] bg-white shadow-lg rounded-xl border border-gray-200 p-6 text-black ${className}`}>
      ...
      <div className="flex justify-between items-start gap-4">
        <div className="flex gap-3 min-w-0">
          <img
            src={post.avatar ? (post.avatar.startsWith("http") ? post.avatar : `https://hewkad.com:2053${post.avatar}`) : 'https://i.pravatar.cc/150'}
            alt="avatar"
        ...
        <dialog className="modal modal-open">
          <div className="modal-box bg-white p-6 rounded-lg shadow-xl text-black">
            ...
            <div className="flex justify-center mb-6">
              <img
                src={`https://hewkad.com:2053${status === 'Reported' ? post.resolved_file : post.proof_url}`}
                alt="Proof"
